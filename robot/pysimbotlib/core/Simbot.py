    ...
    def __init__(
        ...
        **kwargs
        ...
        self._before_simulation = (
            customfn_before_simulation
            if customfn_before_simulation
            else lambda simbot: None
        )
        self._after_simulation = (
            customfn_after_simulation
            if customfn_after_simulation
            else lambda simbot: None
        )
        ...
        if self.iteration == 0:
            ...
            if self.iteration == self.max_tick:
                ...
                if self.save_wasd_history:
                    ...
                    with open(
                        "history{0}.csv".format(self.simulation_count), "w", newline=""
                    ) as out_file:
                        csv_writer = csv.writer(out_file)
                        csv_writer.writerows(
                            self.history if self.history else [["No history"]]
                        )
    ...
    def is_objective_pos_valid(self, obj):
        ...
        for r in self._robot_list:
            if (
                ...
                or r.pos[0] <= pos[0] + obj.size[0] <= r.pos[0] + r.size[0]
            ) and (
                r.pos[1] <= pos[1] <= r.pos[1] + r.size[1]
                or r.pos[1] <= pos[1] + obj.size[1] <= r.pos[1] + r.size[1]
            ):
                return False
        ...
        for o in self._objective_list:
            if obj == o:
                continue
            if (
                o.pos[0] <= pos[0] <= o.pos[0] + o.size[0]
                or o.pos[0] <= pos[0] + obj.size[0] <= o.pos[0] + o.size[0]
            ) and (
                o.pos[1] <= pos[1] <= o.pos[1] + o.size[1]
                or o.pos[1] <= pos[1] + obj.size[1] <= o.pos[1] + o.size[1]
            ):
                return False
...
class PySimbotMap(Widget):
    def __init__(
        self, simbot, enable_wasd_control=False, save_wasd_history=False, **kwargs
    ):
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        ...
        if keycode[1] == "n":
            for obj in self.simbot.objectives:
                ...
                self.simbot.score = int(
                    self.simbot.eat_count * 100 / self.simbot.food_move_count
                )

...
class MyRobot(Robot):
    ...
    def fuzzy_inference_2(self, sensors):
        ...
        if side_count == 4:
            ...
            if side_appear[0] == 0 and side_appear[1] == 1:
            ...
            elif side_appear[0] == 1 and side_appear[1] == 2:
                if smell_side == 1:
            ...
            elif side_appear[0] == 2 and side_appear[1] == 3:
                if smell_side == 2:
                    if back_right_result[1] == 0:
                        turn = 135 - self.defuzzify_turn(back_right)
                ...
                if smell_side == 0:
                    turn = self.defuzzify_turn(self.fuzzy_inference(front)) if smell_half_side else -self.defuzzify_turn(self.fuzzy_inference(front))
                elif smell_side == 2:
                    turn = 180 - self.defuzzify_turn(self.fuzzy_inference(front)) if smell_half_side else -180+self.defuzzify_turn(self.fuzzy_inference(front))
                else:
                    turn = self.smell()
            elif self.stuck and side_appear[0] == 1 and side_appear[1] == 3:
                if smell_side == 1:
                    turn = 90-self.defuzzify_turn(self.fuzzy_inference(front)) if smell_half_side else 90+self.defuzzify_turn(self.fuzzy_inference(front))
                elif smell_side == 3:
                    turn = -90-self.defuzzify_turn(self.fuzzy_inference(front)) if smell_half_side else -90+self.defuzzify_turn(self.fuzzy_inference(front))
                else:
                    turn = self.smell()
        elif side_count == 1 or side_appear_all == 0:
            if front_result[1] == 0:
                ...
                turn = 45-self.defuzzify_turn(front_right)
            elif right_result[1] == 0 and smell_side == 1:
                turn = 90-self.defuzzify_turn(right)
            elif front_left_result[1] == 0:
                turn = -45+self.defuzzify_turn(front_left)
            elif left_result[1] == 0 and smell_side == 3:
                turn = -90+self.defuzzify_turn(left)
        elif side_count == 0:
            turn = self.smell()        
